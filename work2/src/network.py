...
class Network(Layer):
    ...
    def forward(self, seq, dot):
        ...
        emb = paddle.fluid.layers.fc(emb, size=self.model_size)
        emb = paddle.fluid.layers.swish(emb)
        for _ in range(self.layers):
           ...
           # emb = paddle.fluid.layers.fc(emb, size=self.model_size, act="relu")
            emb = paddle.fluid.layers.fc(emb, size=self.model_size)
            emb = paddle.fluid.layers.swish(emb)
        ff_out = paddle.fluid.layers.fc(emb, size=2)
        ff_out = paddle.fluid.layers.leaky_relu(ff_out)
